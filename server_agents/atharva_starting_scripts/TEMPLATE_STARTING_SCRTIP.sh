...
function getPid(){
	...
	sqlite_query "$databaseName" "UPDATE $tblRunTransaction SET pid=$pid_for_job, logTraceFile='$unique_stace_op_Location';"
	if [[ $? -ne 0 ]]; then
                ovarall_execution="fail"
...
unique_stace_op_Location="$trace_logs_location/${run_id}_${job_name}"
...
if [[ $ovarall_execution == "fail" ]]; then
	...
	curr_date=$(date +"%m-%d-%Y %T")
	sqlite_query "$databaseName" "UPDATE $tblRunTransaction SET status='$STAT_COMP_FAIL', end_time='$curr_date', message='FAIL-$failure_message';"
	...
	if [[ $rc -ne 0 ]]; then
		...
		curr_date=$(date +"%m-%d-%Y %T")
        	sqlite_query "$databaseName" "UPDATE $tblRunTransaction SET status='$STAT_COMP', end_time='$curr_date', message='NA';"
        	sqlite_query "$databaseName" "DELETE FROM $tblCurrentRunningJob WHERE run_id=$run_id ;"
		rc=$?
                if [[ $rc -ne 0 ]]; then
			echo "==>Error: Failed to delete table $tblCurrentRunningJob, exiting"
